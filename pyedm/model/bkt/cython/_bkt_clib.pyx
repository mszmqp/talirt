...
def fit(int[:] data,
    ...
    for iter in range(max_iter):
        ...
        for k in range(n_x):
            ...
            x_pos += n_samples
            # 当前观测序列的长度
            n_samples = data_slice[k]
            if n_samples <= 1:
                continue
            x = data[x_pos: x_pos+n_samples]
            # print(n_samples,x[0])
            # 计算前向算法，注意是加了log后的结果
            ...
            for t in range(n_samples-1):
                ...
                # print(t,"alpha", expl(fwdlattice[t][0]),expl(fwdlattice[t][1]))
                # print(t,"beta", expl(bwdlattice[t][0]),expl(bwdlattice[t][1]))
                # print(t,"gamma",gamma[t,0],gamma[t,1])
            # quit(0)
            for i in range(n_stats):
