...
def icc_hpc(theta: numpy.ndarray, items: numpy.ndarray) -> numpy.ndarray:
    ...
    D = 1.702
    ret = numexpr.evaluate('c + ((d - c) / (1 + exp((-a*1.702 * (theta - b)))))')
    return ret.flatten()  # ret.reshap((ret.shape[0],))
...
def log_likelihood(
        user_parameters, response_vector: list, item_parameters
) -> float:
    ...
    """
    # if len(response_vector) != user_parameters.shape[0]:
    #     raise ValueError(
    ...
    # item_parameters = convert_item_parameter(item_parameters, len(response_vector))
    # user_parameters = convert_user_paramter(user_parameters, len(response_vector))
    ps = icc_hpc(user_parameters, item_parameters)
...
def negative_log_likelihood(parameters, *args) -> float:
    ...
    if param_type == "item":
        # a = parameters[0]
        # b = parameters[1]
        # item_parameters = numpy.array([a,b,0,1])
        item_parameters = parameters
        user_parameters, response_vector = args[:2]
    elif param_type == 'user':
        user_parameters = parameters
        item_parameters, response_vector = args[:2]
    # size = len(response_vector)

    return -log_likelihood(user_parameters=user_parameters, response_vector=response_vector,
